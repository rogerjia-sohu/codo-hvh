...
use hvh\{ImageFileManager};

$pagename = 'Android App PicUpload';
printf('<head><title>%s</title></head>', $pagename);
printf('%s<p/>', $pagename);

if (!empty($_FILES)) {
	$fm = new ImageFileManager($_SERVER['DOCUMENT_ROOT'], 'images/androidapp');
	...
	foreach ($filenames as $fname) {
		if (!empty($fname)) {
			...
			$dummy = '';
			$ret = $fm->SaveFile($tmpfile_w_orgext, null, true, $dummy, 'wxapp');
